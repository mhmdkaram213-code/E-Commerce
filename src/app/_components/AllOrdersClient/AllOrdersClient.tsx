...
export default function AllOrdersClient() {
  ...
  useEffect(() => {
    ...
    async function getOrders() {
      try {
        if (!session) return
        const res = await fetch(`https://ecommerce.routemisr.com/api/v1/orders/user/${session.user._id}`,
          {
            headers: {
  ...
  if (loading) {
    return (
      <div className="h-screen flex items-center justify-center">
        <p className="animate-pulse text-green-600 text-lg">Loading orders...</p>
      </div>
    )
  }
  return (
      ...
      <div className="text-gray-800  dark:bg-gray-900 hover:bg-gray-100  hover:dark:bg-gray-800 rounded-2xl shadow-md p-6 border border-gray-200 dark:text-white dark:border-gray-700">
        ...
        {orders.length === 0 ? (
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-5 px-4 pb-4 pt-20">
            {orders.map(order => (
              <div key={order._id} className="p-6 bg-white rounded-xl shadow space-y-2 text-sm text-gray-700 dark:text-white">
