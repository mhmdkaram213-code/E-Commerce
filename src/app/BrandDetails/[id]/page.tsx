...
export default async function BrandDetails(props: myProps) {
    ...
    const { data: brand }: { data: BrandsItem } = await req.json()
    return (
        <div className="grid md:grid-cols-3 gap-8 items-center p-6 justify-center">
            <div className="md:col-span-1 border border-gray-50 shadow-xl">
                <Image src={brand.image} alt={brand.name} className='w-full' width={200} height={300} />
            </div>
            <div className="md:col-span-2 border border-gray-50 shadow-xl">
                <Card className="mb-12">
                        <CardHeader className="flex flex-col items-center gap-4">
                            <CardTitle className="text-3xl">{brand.name?.split(" ").slice(0, 2).join(" ")}</CardTitle>
                            <Badge variant="secondary">{brand.slug}</Badge>
                        </CardHeader>
                        <CardContent className="text-center text-sm text-muted-foreground">
                            <p>Created At: {new Date(brand.createdAt).toLocaleDateString()}</p>
                            <p>Last Update: {new Date(brand.updatedAt).toLocaleDateString()}</p>
                        </CardContent>
                    <CardFooter>
